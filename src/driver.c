...
static bool is_generated_zero_pes(const char *value)
{
    if (value == NULL) return false;
    return strcasecmp(value, "GENERATE:ZERO") == 0 ||
           strcasecmp(value, "ZERO") == 0;
}

static bool generate_zero_pes(const char *project_dir, size_t q_size, char **so_path_out)
{
    static const char *source_filename = "generated_zero_pes.c";
    static const char *library_filename = "generated_zero_pes.so";

    char *source_path = prefix_with_dir(project_dir, source_filename);
    char *library_path = prefix_with_dir(project_dir, library_filename);
    bool result = false;

    PRINT0("\n");
    PRINT0("*****************************************************\n");
    PRINT0("Generating requested ZERO potential\n");
    PRINT0("Generalized-coordinate count: %zu\n", q_size);
    PRINT0("Generated source:             %s\n", source_path);
    PRINT0("Generated shared library:     %s\n", library_path);
    PRINT0("Compiler invocation:          cc -shared -fPIC -O2 %s -o %s\n",
    ...
    if (fp == NULL) {
        ERROR("Failed to create generated zero-PES source '%s': %s\n",
    ...
    int written = fprintf(
        ...
        "{\n"
        "    (void) q;\n"
        "    for (size_t i = 0; i < %zu; ++i) dpesdq[i] = 0.0;\n"
        "}\n",
        q_size);

    if (written < 0 || fclose(fp) != 0) {
        fp = NULL;
        ERROR("Failed to write generated zero-PES source '%s': %s\n",
              source_path, strerror(errno));
        goto defer;
    }
    fp = NULL;
    ...
    if (compiler_pid < 0) {
        ERROR("Failed to start C compiler for generated zero PES: %s\n",
              strerror(errno));
        goto defer;
    }

    if (compiler_pid == 0) {
        execlp("cc", "cc", "-shared", "-fPIC", "-O2", source_path,
               "-o", library_path, (char *) NULL);
        fprintf(stderr, "ERROR: failed to execute 'cc' for generated zero PES: %s\n",
                strerror(errno));
        _exit(127);
    }

    int compiler_status = 0;
    if (waitpid(compiler_pid, &compiler_status, 0) < 0) {
        ERROR("Failed while waiting for zero-PES compiler: %s\n", strerror(errno));
        goto defer;
    }

    if (!WIFEXITED(compiler_status) || WEXITSTATUS(compiler_status) != 0) {
        ERROR("C compiler failed while generating '%s'\n", library_path);
        goto defer;
    }

    INFO("Generated zero PES for %zu generalized coordinates: %s\n",
         q_size, library_path);
    *so_path_out = library_path;
    library_path = NULL;
    result = true;
...
int main(int argc, char* argv[]) 
{
    ...
    if (*check) {
        ...
        if (is_generated_zero_pes(input_block.so_potential)) {
            INFO("%s: recognized generated zero-PES specification; "
                 "the library will be created when the calculation runs\n",
                 input_block.so_potential);
        } else if (input_block.so_potential != NULL) {
    ...
    if (is_generated_zero_pes(input_block.so_potential)) {
        if (oc.project_name == NULL) {
            ERROR("A project directory is required for a generated PES\n");
            return_defer(1);
        }

        size_t q_size = 3 + (monomer1.t % MODULO_BASE) / 2
                          + (monomer2.t % MODULO_BASE) / 2;
        char *generated_so_path = prefix_with_dir(oc.project_name,
                                                   "generated_zero_pes.so");
        int generation_ok = 1;

        if (_wrank == 0) {
            char *compiled_path = NULL;
            generation_ok = generate_zero_pes(oc.project_name, q_size,
                                               &compiled_path);
            if (generation_ok) {
                free(generated_so_path);
                generated_so_path = compiled_path;
            }
        }

        MPI_Bcast(&generation_ok, 1, MPI_INT, 0, MPI_COMM_WORLD);
        if (!generation_ok) {
            free(generated_so_path);
            return_defer(1);
        }

        input_block.so_potential = generated_so_path;
    }
    ...
    switch (calc_params.calculation_type) {
        ...
        case CALCULATION_PR_MU_TRANSITION_FREQUENCY_SAMPLING: {
            if (strcmp(input_block.so_dipole_1, input_block.so_dipole_2) != 0) {
                PRINT0("ERROR: for PR_MU_TRANSITION_FREQUENCY_SAMPLING calculation we expect to have only one dipole\n");
                exit(1);
            }
            ...
            MoleculeSystem ms = init_ms_from_monomers(input_block.reduced_mass, &monomer1, &monomer2, 0);
            SFnc sf = calculate_spectral_function_using_prmu_transition_frequency_sampling_and_save(&ms, &calc_params, input_block.Temperature);
