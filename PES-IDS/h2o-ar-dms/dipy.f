...
!**************************************************************************
        subroutine h2oardipNNy(xct,dip,dipa,dipb,dipc)
        use nnparamy
        implicit none
        ...
        call getpotya(txinput,dipa)
        call getpotyb(txinput,dipb)
        call getpotyc(txinput,dipc)
        dip=(dipa+dipb+dipc)/3.0d0

        return
        end subroutine h2oardipNNy
!**************************************************************************
...
!the getpot() subroutine
     ...
     %  'PES-IDS/h2o-ar-dms/'
        ...
        open(wfilea,file=DMSDIR//'weightsya.txt',status='old')
        open(bfilea,file=DMSDIR//'biasesya.txt',status='old')
     ...
     %  biasa(nodemax,2:nlayer))
        ...
        open(wfileb,file=DMSDIR//'weightsyb.txt',status='old')
        open(bfileb,file=DMSDIR//'biasesyb.txt',status='old')
     ...
     %  biasb(nodemax,2:nlayer))
        ...
        open(wfilec,file=DMSDIR//'weightsyc.txt',status='old')
        open(bfilec,file=DMSDIR//'biasesyc.txt',status='old')
     ...
     %  biasc(nodemax,2:nlayer))
        ...
        end subroutine dipnn_inity
!**************************************************************************


!**************************************************************************
        subroutine getpotya(x,vpot)
        use nnparamy
        implicit none
...
!-->....the value of output layer is the fitted potntial 
        ...
        return
        end subroutine getpotya
!**************************************************************************


!**************************************************************************
        subroutine getpotyb(x,vpot)
        use nnparamy
        implicit none
        integer i,inode1,inode2,ilay1,ilay2
        real*8 x(ninput),y(nodemax,nlayer),vpot
...
!-->....the value of output layer is the fitted potntial 
        ...
        return
        end subroutine getpotyb
!**************************************************************************


!**************************************************************************
        subroutine getpotyc(x,vpot)
        use nnparamy
        implicit none
        integer i,inode1,inode2,ilay1,ilay2
        real*8 x(ninput),y(nodemax,nlayer),vpot
